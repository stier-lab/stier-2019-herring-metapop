...
### You are calculating confidence intervals using some central limit assumptions.
### The MCMC samples are draws from the posterior.  

# Therefore for each draw of pdocoef:
#   multiply pdocoef by the whole pdo time series, 
#   repeat for all draws, 
#   THEN summarize.
...
pdotsgg<-ggplot(pdodf2,aes(x=year,y=Median))+
  geom_hline(yintercept=0)+
  geom_ribbon(aes(ymin=x.05,ymax=x.95),fill="grey70",alpha=0.2)+
  geom_ribbon(aes(ymin=x.25,ymax=x.75),fill="grey")+
  geom_line(lty=2)+
...
# what the coefficient is ("ANYTHING") or its uncertainty.  It'll always be close to 0, 

# However, I think the plotting looked weird because everytime you switched from positive to negative PDO, 
# the values were crossing (25% interval was closer to zero when PDO was positive AND when PDO negative)
# So the confidence interval crossed and looked like they were == 0... so it was a plotting artifact.
